// IMPORTS - React, Leaflet, Bibliotecas
...
function AreaLogada({ telaAtual, setTelaAtual, menuUsuarioAberto, setMenuUsuarioAberto, carrinhoAberto, setCarrinhoAberto, itensCarrinho, setItensCarrinho, modalEnderecoAberto, setModalEnderecoAberto, setPassoEndereco, lojaSelecionada, setLojaSelecionada, lojas, produtoSelecionado, setProdutoSelecionado, quantidadeProduto, setQuantidadeProduto, passoEndereco, tipoFavorito, setTipoFavorito, usuarioNome, setUsuarioNome }) {
  ...
  const [tipoEnvio, setTipoEnvio] = useState('entrega');
  ...
  const [dadosEntrega, setDadosEntrega] = useState({
    taxaEntrega: 0, isFreteGratis: false, rua: '', cidadeEstado: '', tempoEstimado: ''
  });
  ...
  // Nome de exibição (prioridade: prop > localStorage)
  const nomeExibido = (() => {
    ...
    try {
      ...
      if (usuarioStorage.nome) return usuarioStorage.nome.split(' ')[0];
    } catch (e) {}
    return 'Usuário';
  })();
  ...
  // Busca endereços por CEP (8 dígitos) ou texto livre
  const buscarEnderecos = async (texto) => {
    ...
    if (valorLimpo.length === 8) {
      try {
        ...
        if (!dados.erro) {
          setSugestoes([{ display_name: `${dados.logradouro}, ${dados.bairro} - ${dados.localidade}/${dados.uf}`, isCep: true, ...dados }]);
          return;
        }
      } catch (err) { console.error("Erro na busca por CEP", err); }
    }
  ...
  // Controla input de busca com debounce (500ms)
  const onChangeBusca = (e) => {
    const valor = e.target.value;
    setBusca(valor);
    if (debounceTimer.current) clearTimeout(debounceTimer.current);
    if (valor.length > 3) {
      debounceTimer.current = setTimeout(() => buscarEnderecos(valor), 500);
    } else {
      setSugestoes([]);
    }
  };
  ...
  // Confirma número e avança para visualização no mapa
  const confirmarNumeroEAvancar = async () => {
    if (!semNumero && numero && enderecoSelecionado) {
      ...
      try {
        let res = await fetch(`http://127.0.0.1:5000/enderecos/buscar-sugestoes?q=${encodeURIComponent(`${logradouro}, ${numero}, ${cidade}, Brasil`)}`);
        let dados = await res.json();
        if (dados?.[0]) {
          setEnderecoSelecionado(prev => ({ ...prev, lat: parseFloat(dados[0].lat), lon: parseFloat(dados[0].lon) }));
        } else {
          let resRua = await fetch(`http://127.0.0.1:5000/enderecos/buscar-sugestoes?q=${encodeURIComponent(`${logradouro}, ${cidade}, Brasil`)}`);
          let dadosRua = await resRua.json();
          if (dadosRua?.[0]) setEnderecoSelecionado(prev => ({ ...prev, lat: parseFloat(dadosRua[0].lat), lon: parseFloat(dadosRua[0].lon) }));
        }
  ...
  // Salva endereço no banco e atualiza taxa de entrega
  const salvarEnderecoCompleto = async () => {
    const idLogado = localStorage.getItem('usuario_id');
    if (!idLogado) { alert("Usuário não identificado."); return; }

    const payload = {
      usuario_id: parseInt(idLogado), cep: enderecoSelecionado.cep || '00000-000',
      logradouro: enderecoSelecionado.logradouro, numero: semNumero ? 'S/N' : numero,
      bairro: enderecoSelecionado.bairro, cidade: enderecoSelecionado.localidade || enderecoSelecionado.cidade,
      estado: enderecoSelecionado.uf || enderecoSelecionado.estado, complemento, ponto_referencial: referencia,
      rotulo: tipoFavorito, latitude: enderecoSelecionado.lat, longitude: enderecoSelecionado.lon, principal: true
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDadosEntrega({ rua: `${resultado.logradouro}, ${resultado.numero}`, cidadeEstado: `${resultado.cidade} - ${resultado.estado}`, tempoEstimado: "30-40 min", taxaEntrega: 0, isFreteGratis: false });
  ...
  const buscarEnderecosDoUsuario = async () => {
    ...
    try {
      ...
      if (response.ok) { setListaEnderecos(dados); setModalEnderecos(true); }
      else alert("Erro ao buscar endereços");
    } catch (error) { console.error("Erro ao buscar endereços:", error); }
  };

  // Seleciona um endereço da lista e recalcula taxa
  const selecionarEndereco = async (endereco) => {
    setEnderecoSelecionado(endereco);
    setDadosEntrega({ rua: `${endereco.logradouro}, ${endereco.numero || 'S/N'}`, cidadeEstado: `${endereco.cidade} - ${endereco.estado}`, taxaEntrega: 0, isFreteGratis: false, tempoEstimado: '' });
    setModalEnderecos(false);
    if (pedidoAtivoId) await atualizarTaxaEntrega(endereco.id, pedidoAtivoId);
  };

  // Calcula taxa de entrega via API
  const atualizarTaxaEntrega = async (enderecoId, pedidoIdOuRestauranteId) => {
    try {
      const response = await fetch('http://localhost:5000/pagamentos/calcular-taxa', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          endereco_id: enderecoId,
          ...(pedidoIdOuRestauranteId > 0 ? { pedido_id: pedidoIdOuRestauranteId } : { restaurante_id: itensCarrinho[0]?.restaurante_id })
        })
      });
      const data = await response.json();
      if (data.taxaEntrega !== undefined) {
        setDadosEntrega(prev => ({ ...prev, taxaEntrega: data.taxaEntrega, isFreteGratis: data.isFreteGratis || false, tempoEstimado: data.tempoEstimado || '' }));
      }
    } catch (error) { console.error("Erro ao calcular taxa:", error); }
  };


  // FUNÇÕES - CARRINHO

  // Recupera itens do carrinho pelo ID do pedido
  const recuperarCarrinho = async (id) => {
    const res = await fetch(`http://127.0.0.1:5000/pedidos/${id}/status`);
    if (res.ok) setItensCarrinho((await res.json()).itens);
  };

  // Adiciona item ao carrinho (cria pedido se necessário)
  const handleAdicionarAoCarrinho = async () => {
    try {
      let idCarrinho = pedidoAtivoId;
      if (!idCarrinho) {
        const usuarioId = localStorage.getItem('usuario_id');
        if (!usuarioId) { alert("Usuário não identificado."); return; }
        const resCarrinho = await fetch("http://127.0.0.1:5000/pedidos", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ restaurante_id: lojaSelecionada.id, usuario_id: parseInt(usuarioId) })
        });
      ...
      const resItem = await fetch(`http://127.0.0.1:5000/pedidos/${idCarrinho}/itens`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ doce_id: produtoSelecionado.id, quantidade: quantidadeProduto, observacao: comentario })
      });
  ...
  const removerItemDoBanco = async (idItem) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:5000/pedidos/itens/${idReal}`, { method: "DELETE" });
      if (res.ok) setItensCarrinho((await res.json()).itens);
  ...
  // Inicia o checkout (entrega ou retirada)
  const handleFazerPedido = async () => {
    if (tipoEnvio === 'entrega' && (!enderecoSelecionado?.id)) {
      alert("Selecione um endereço antes!"); return;
    }
    ...
    try {
      ...
      const endpoint = tipoEnvio === 'retirada' ? 'http://127.0.0.1:5000/pagamentos/checkout-retirada' : 'http://127.0.0.1:5000/pagamentos/checkout';
      const dadosCheckout = {
        pedido_id: pedidoAtivoId, metodo_id: abaPagamento === 'site' ? 1 : 3, tipo_envio: tipoEnvio, subtotal, email_cliente: "cliente@venda.com",
        ...(tipoEnvio === 'entrega' && { endereco_id: enderecoSelecionado.id })
      };

      const response = await fetch(endpoint, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(dadosCheckout)
      });
      const dadosRetorno = await response.json();

      if (response.ok) {
        if (dadosRetorno.checkout_url) {
          setTimeout(() => { window.location.href = dadosRetorno.checkout_url; }, 1500);
        } else {
          ...
          if (tipoEnvio === 'retirada') toast.success('✅ Pedido confirmado!');
          setTelaAtual('pedidos');
        }
      } else {
        alert("Erro no checkout: " + dadosRetorno.erro);
        setStatusPagamento('erro');
      }
    } catch (erro) { console.error("Erro no checkout:", erro); setStatusPagamento('erro'); }
  };
  ...
  const confirmarEntregaFinal = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/pedido/${pedidoAtual.id}/confirmar-entrega`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ codigo_verificacao: codigo })
      });
      if (response.ok) {
        toast.success('✅ Pedido entregue!');
        localStorage.removeItem('pop_pedido_id');
        setPedidoAtual(null); setItensCarrinho([]); setPedidoAtivoId(null);
        await carregarHistoricoPedidos();
      } else {
        alert((await response.json()).erro);
      }
    } catch (e) { console.error("Erro ao confirmar entrega:", e); }
  };

  // FUNÇÕES - STATUS PEDIDOS

  // Carrega pedidos do usuário (atuais e anteriores)
  const carregarHistoricoPedidos = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPedidosAnteriores(dados.filter(p => ['Entregue', 'CANCELADO'].includes(p.status)));
        setInfoEntregador(atual?.status === 'Em Trânsito' ? atual.detalhes_entrega?.entregador || null : null);
      }
    } catch (error) { console.error("Erro ao buscar histórico:", error); }
  };

  // Atualiza status do pedido atual (pooling)
  const atualizarStatusPedido = async () => {
    ...
    try {
      ...
      const dados = await res.json();
      if (res.ok && dados.status !== statusAnterior) {
        setPedidoAtual(dados);
        const notificacoes = {
          'Em Preparação': '🍰 Seu pedido está em preparação!',
          'Pronto': '🧁 Seu pedido está pronto!',
          'Em Trânsito': '🛵 Seu pedido saiu para entrega!',
          'Entregue': '✅ Pedido entregue!'
        };
        if (notificacoes[dados.status]) toast.success(notificacoes[dados.status]);
  ...
  // Confirma retirada do pedido na loja
  const confirmarRetirada = async () => {
    if (!pedidoAtual) return;
    try {
      const response = await fetch(`http://127.0.0.1:5000/pedidos/${pedidoAtual.id}/confirmar-retirada`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ senha: pedidoAtual.numero_senha })
      });
      if (response.ok) {
        toast.success('✅ Pedido retirado! Bom apetite! 🍩');
        await carregarHistoricoPedidos();
      } else {
        toast.error((await response.json()).erro || 'Erro ao confirmar retirada');
      }
    } catch (error) { console.error("Erro na retirada:", error); }
  };

  // FUNÇÕES - CANCELAR PEDIDO

  // Cancela o pedido ativo
  const cancelarPedido = async () => {
    if (!pedidoAtivoId) { alert("Nenhum pedido ativo."); return; }
    if (!window.confirm("Cancelar este pedido?")) return;
    try {
      const response = await fetch(`http://127.0.0.1:5000/pedidos/${pedidoAtivoId}/cancelar`, { method: "PUT", headers: { "Content-Type": "application/json" } });
      if (response.ok) {
        setItensCarrinho([]); setPedidoAtivoId(null); localStorage.removeItem('pop_pedido_id');
        setDadosEntrega({ taxaEntrega: 0, isFreteGratis: false, rua: '', cidadeEstado: '', tempoEstimado: '' });
        setTelaAtual('dashboard');
      } else {
        alert("Erro: " + (await response.json()).erro);
      }
  ...
  // Carrega produtos e lojas ao montar
  useEffect(() => {
    carregarProdutos();
  ...
  // Recupera pedido ativo do localStorage
  useEffect(() => {
    const idSalvo = localStorage.getItem('pop_pedido_id');
    if (idSalvo) { const idNum = parseInt(idSalvo); setPedidoAtivoId(idNum); recuperarCarrinho(idNum); }
  }, []);

  // Sincroniza pedidoAtivoId com localStorage
  useEffect(() => {
    pedidoAtivoId ? localStorage.setItem('pop_pedido_id', pedidoAtivoId) : localStorage.removeItem('pop_pedido_id');
  }, [pedidoAtivoId]);

  // Verifica status do pagamento a cada 5s
  useEffect(() => {
    if (!pedidoAtivoId) return;
    const verificarPagamento = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:5000/pagamentos/pedido/${pedidoAtivoId}`);
        const dados = await response.json();
        if (dados.status === "Pagamento Confirmado") {
          setItensCarrinho([]); setPedidoAtivoId(null); localStorage.removeItem("pop_pedido_id"); carregarHistoricoPedidos();
        }
      } catch (error) { console.error("Erro verificando pagamento:", error); }
    };
    verificarPagamento();
  ...
  // Carrega histórico ao entrar na tela de pedidos
  useEffect(() => { if (telaAtual === 'pedidos') carregarHistoricoPedidos(); }, [telaAtual]);

  // Atualiza status do pedido a cada 10s
  useEffect(() => {
    if (!pedidoAtual) return;
    const intervalo = setInterval(atualizarStatusPedido, 10000);
    return () => clearInterval(intervalo);
  }, [pedidoAtual]);

  // Recalcula taxa quando endereço ou carrinho mudam
  useEffect(() => {
    if (enderecoSelecionado?.id && pedidoAtivoId) atualizarTaxaEntrega(enderecoSelecionado.id, pedidoAtivoId);
  }, [enderecoSelecionado, pedidoAtivoId, itensCarrinho]);

  // Carrega endereço principal do usuário
  useEffect(() => {
    (async () => {
      const usuarioId = localStorage.getItem('usuario_id');
      if (!usuarioId) return;
      try {
        ...
        if (response.ok && enderecos.length > 0) {
          ...
          setDadosEntrega({
            rua: `${principal.logradouro}, ${principal.numero}`, cidadeEstado: `${principal.cidade} - ${principal.estado}`,
            taxaEntrega: principal.taxa_entrega_pre_calculada || 0, isFreteGratis: false
          });
        }
    ...
    (async () => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setUsuario(data);
          if (data.nome) { setUsuarioNome(data.nome); localStorage.setItem('usuario_nome', data.nome); localStorage.setItem('usuario', JSON.stringify(data)); }
        }
  ...
  useEffect(() => {
    const handleUsuarioAtualizado = (evento) => {
      ...
      if (dadosAtualizados.nome && setUsuarioNome) setUsuarioNome(dadosAtualizados.nome);
      setUsuario(prev => ({ ...prev, ...dadosAtualizados }));
    };
    window.addEventListener('usuario-atualizado', handleUsuarioAtualizado);
    return () => window.removeEventListener('usuario-atualizado', handleUsuarioAtualizado);
  }, [setUsuarioNome]);
    ...
    <>
      ...
      {(telaAtual === "dashboard" || telaAtual === "pedidos" || telaAtual === "tela-restaurante" || telaAtual === "pagamento") && (
        <div style={{ width: "100%", minHeight: "100vh", backgroundColor: "#fff" }}>
          ...
          <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "15px 40px", backgroundColor: "#ffe6e8", }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "25px" }}>
              ...
              <div style={{ position: "relative" }}>
                <span style={{ color: "#ff3b3b", fontSize: "1.8rem", cursor: "pointer", }} onClick={() => setMenuUsuarioAberto(!menuUsuarioAberto)}> 👤 </span>
              ...
              <div style={{ position: "relative" }}>
                <span onClick={() => setCarrinhoAberto(!carrinhoAberto)} style={{ cursor: "pointer", fontSize: "1.5rem", color: "#ff3b3b", }} title="Ver carrinho de pedidos"> 🛒 </span>

                {/*==============================CARRINHO========================================================================== */}
                {carrinhoAberto && (
                  <div className="popup-carrinho" style={{ position: "absolute", top: "50px", right: "0", width: "380px", backgroundColor: "#fff", borderRadius: "10px", padding: "20px", boxShadow: "0 10px 30px rgba(0,0,0,0.15)", zIndex: 999, }}>
                    ...
                    {itensCarrinho.length === 0 ? (
                      ...
                      <div className="carrinho-vazio" style={{ textAlign: "center", padding: "10px 0 30px 0", }}>
                        <img src={imgFormigaDormindo} alt="Formiguinha fofa" style={{ width: "180px", height: "auto", marginBottom: "20px", }} />
                        <h3 style={{ color: "#555", fontWeight: "bold", margin: "0 0 10px 0", fontSize: "1.2rem", }}> Sua sacola está vazia </h3>
                        <p style={{ color: "#999", margin: 0, fontSize: "0.9rem", }}> adicione um docinho aqui </p>
                      </div>
                    ) : (
                      /* --- ESTADO COM ITENS (DINÂMICO) --- */
                      <div className="carrinho-cheio">
                        <div className="carrinho-restaurante" style={{ marginBottom: "15px" }}>
                          <p style={{ color: "#999", fontSize: "0.9rem", margin: "0 0 5px 0", }}> Seu pedido em{" "}
                            <span style={{ color: "#ff3b3b", float: "right", cursor: "pointer", }}> Ver o cardápio </span>
                          </p>
                          ...
                          {itensCarrinho.map((item, index) => (
                            <div key={index} style={{ marginBottom: "20px" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "5px", }}>
                                <span style={{ color: "#8a1c1c", fontWeight: "bold", fontSize: "1.1rem", }}> {item.quantidade} x {item.nome_doce} </span>
                                <span style={{ fontWeight: "bold", fontSize: "1.1rem", }}> R$ {item.subtotal.toFixed(2).replace(".", ",")} </span>
                              </div>
                              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.9rem", }}>
                                ...
                                <div style={{ display: "flex", gap: "15px" }}>
                                  ...
                                  <span onClick={() => removerItemDoBanco(item.id)} style={{ color: "#000", fontWeight: "bold", cursor: "pointer", }}> Remover </span>
                                </div>
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="carrinho-resumo" style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px", }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", color: "#999", fontWeight: "bold", }}>
                            ...
                            <span style={{ color: "#00b894" }}>
                              {" "} {dadosEntrega.isFreteGratis ? "Grátis" : `R$ ${Number(dadosEntrega.taxaEntrega || 0) .toFixed(2) .replace(".", ",")}`}
                            </span>
                          </div>
                          <div style={{ display: "flex", justifyContent: "space-between", color: "#000", fontWeight: "bold", fontSize: "1.3rem", marginTop: "10px", }}>
                            <span>Total</span>
                            <span>R$ {(itensCarrinho.reduce((acc, i) => acc + i.preco_unitario * i.quantidade, 0) + (dadosEntrega.isFreteGratis ? 0 : Number(dadosEntrega.taxaEntrega))).toFixed(2).replace(".", ",")}</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          onClick={() => { if (itensCarrinho.length > 0) { setTelaAtual("pagamento"); } else { alert("Seu carrinho está vazio!"); } }}>
                          Escolher forma de pagamento
                        </button>
                        <button onClick={() => setItensCarrinho([])} style={{ width: "100%", backgroundColor: "#f5f5f5", color: "#000", border: "1px dashed #ccc", padding: "10px", borderRadius: "8px", cursor: "pointer", }}> Esvaziar </button>
                      </div>
                    )}
                  </div>
                )}
          ...
          <main className="conteudo-dashboard" style={{ padding: "40px", maxWidth: "1000px", margin: "0 auto" }}>
              ...
              <div className="container-pagamento">
                ...
                <div className="pagamento-coluna-esq">
                  ...
                  <div className="abas-simples">
                    <span className={tipoEnvio === "entrega" ? "aba-ativa" : "aba-inativa"} onClick={() => setTipoEnvio("entrega")} style={{ cursor: "pointer" }}>
                      Entrega
                    </span>
                    <span className={tipoEnvio === "retirada" ? "aba-ativa" : "aba-inativa"} onClick={() => setTipoEnvio("retirada")} style={{ cursor: "pointer" }}>
                      Retirada no Local
                    </span>
                  </div>
                  ...
                  {/* Botão Cancelar */}
                  <button onClick={cancelarPedido} style={{ width: "100%", padding: "12px", marginTop: "15px", backgroundColor: "#fff", color: "#ff3b3b", border: "2px solid #ff3b3b", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", fontSize: "1rem", transition: "all 0.2s", }}
                    onMouseEnter={e => {
                ...
                <div className="pagamento-coluna-dir" style={{ minWidth: "380px", width: "35%" }}>
                  <div className="resumo-pedido-fixo">
                    <p className="texto-cinza" style={{ color: "#999", fontSize: "0.9rem", marginBottom: "5px", }}> Seu pedido em{" "}
                      <span style={{ color: "#ff3b3b", float: "right", fontSize: "0.8rem", cursor: "pointer", fontWeight: "bold", }} onClick={() => setTelaAtual("tela-restaurante")}> Ver o cardápio </span>
                    </p>
                    ...
                    <div style={{ marginTop: "15px" }}>
                      ...
                      <div className="linha-resumo" style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px", color: "#666", }}>
                        ...
                        <span style={{ color: "#2ecc71", fontWeight: "bold" }}>
                          {tipoEnvio === "retirada" ? "Grátis" : dadosEntrega.isFreteGratis ? "Grátis" : `R$ ${Number(dadosEntrega.taxaEntrega || 0) .toFixed(2) .replace(".", ",")}`}
                        </span>
                      </div>
            ...
            {telaAtual === "dashboard" && (
              <div style={{ width: "100%", fontFamily: "sans-serif" }}>
                ...
                <section style={{ marginBottom: "50px" }}>
                  <h3 style={{ fontSize: "1.2rem", fontWeight: "bold", marginBottom: "15px", color: "#000", }}> Pedir seu docinho no POP! é rápido, fácil e gostoso. Conheça algumas categorias </h3>

                  {/* Faixa Rosa com as categorias */}
                  <div style={{ backgroundColor: "#ffe6e8", borderRadius: "15px", padding: "20px 30px", display: "flex", alignItems: "center", position: "relative", }}>
                    ...
                    <div style={{ width: "40px", height: "40px", border: "2px solid #fff", borderRadius: "50%", display: "flex", justifyContent: "center", alignItems: "center", position: "absolute", right: "20px", cursor: "pointer", color: "#fff", fontSize: "1.2rem", }}> &gt; </div>
                  </div>
                </section>
                ...
                <section style={{ marginBottom: "60px" }}>
                  <h3 style={{ textAlign: "center", fontSize: "1.2rem", fontWeight: "bold", marginBottom: "20px", color: "#000", }}> Ofertas especiais no precinho 😉 </h3>

                  {/* Faixa Rosa com as ofertas */}
                  <div style={{ backgroundColor: "#ffe6e8", borderRadius: "15px", padding: "30px", display: "flex", alignItems: "center", position: "relative", }}>
                ...
                <section className="secao-lojas-dash" style={{ marginBottom: "50px" }}>
                  ...
                  {lojas.length === 0 ? (
                    <div style={{ padding: "40px", textAlign: "center", backgroundColor: "#fff", borderRadius: "12px", border: "2px dashed #eaeaea", }}>
                      ...
                      <button onClick={() => setTelaAtual("cadastro-restaurante")} style={{ padding: "12px 25px", backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", fontSize: "1rem", }}> + Cadastrar Restaurante </button>
                    </div>
                  ) : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "40px 20px", }}>
                      {lojas.map(loja => (
                        <div
                          key={loja.id}
                          onClick={() => { console.log("Loja clicada:", loja); setLojaSelecionada(loja); setTelaAtual("tela-restaurante"); }}
                          style={{ display: "flex", alignItems: "center", gap: "15px", cursor: "pointer", transition: "transform 0.2s", }}
                          onMouseEnter={e => {
                          ...
                          <div style={{ width: "70px", height: "70px", backgroundColor: "#fff", borderRadius: "10px", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden", border: "1px solid #eee", }}>
                            <img src={loja.imagem || "URL_DE_UMA_IMAGEM_PADRAO"} alt={loja.nome} style={{ width: "100%", height: "100%", objectFit: "cover", }} />
                          </div>
                  ...
                  {lojas.length > 0 && (
                    <div style={{ display: "flex", justifyContent: "center", marginTop: "50px", }}>
                      <button style={{ padding: "10px 60px", backgroundColor: "#fff", color: "#ff4d6d", border: "1px solid #ff4d6d", borderRadius: "25px", fontSize: "1rem", cursor: "pointer", fontWeight: "bold", }}> Ver mais </button>
                    </div>
                  )}
                </section>
              </div>
            ...
            {telaAtual === "pedidos" && (
              <div style={{ padding: "40px", maxWidth: "900px", margin: "0 auto" }}>
                <button onClick={() => setTelaAtual("dashboard")} style={{ marginBottom: "20px", padding: "8px 15px", borderRadius: "20px", border: "1px solid #ccc", background: "white", cursor: "pointer", fontWeight: "bold", color: "#666", }}> ← Voltar para o início </button>
                ...
                <div style={{ backgroundColor: "#fff5f6", borderRadius: "15px", padding: "40px", boxShadow: "0 4px 15px rgba(0,0,0,0.05)", }}>
                    ...
                    <div style={{ backgroundColor: "#fbeceb", padding: "25px", borderRadius: "15px", marginTop: "10px", }}>
                      <div style={{ display: "flex", gap: "25px" }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: "120px", }}>
                          <div style={{ width: "120px", height: "120px", backgroundColor: "#bdc3c7", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", }}>
                            <img src={pedidoAtual.itens[0]?.imagem || imgPlaceholder} alt="Foto do pedido" style={{ width: "100%", height: "100%", objectFit: "cover", }} />
                          </div>
                          <span style={{ marginTop: "10px", fontWeight: "bold", color: "#333", }}> R$ {calcularTotalComTaxa(pedidoAtual).toFixed(2).replace(".", ",")} </span>
                        </div>

                        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px", }}>
                          <div style={{ display: "flex", justifyContent: "space-between", }}>
                            <span style={{ fontWeight: "bold", color: "#a82424", fontSize: "1.1rem", }}> {pedidoAtual?.restaurante_nome || "Tortarugas"} </span>
                          </div>
                      ...
                      <div style={{ display: "flex", gap: "20px", marginTop: "20px", }}>
                        {pedidoAtual?.tipo_retirada === "retirada" ? (
                          <>
                            <button onClick={() => { setPedidoConfirmadoRetirada({ pedido_id: pedidoAtual.id, numero_senha: pedidoAtual.numero_senha, }); setModalPainelSenhasAberto(true); }} style={{ flex: 1, backgroundColor: "#ff3b3b", color: "#fff", border: "none", padding: "12px", borderRadius: "30px", fontWeight: "bold", fontSize: "1rem", cursor: "pointer", }}> Ver Painel de Senhas </button>
                            <button onClick={confirmarRetirada} style={{ flex: 1, backgroundColor: "#27ae60", color: "#fff", border: "none", padding: "12px", borderRadius: "30px", fontWeight: "bold", fontSize: "1rem", cursor: "pointer", }}> Confirmar Retirada </button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => setModalAcompanharAberto(true)} style={{ flex: 1, backgroundColor: "#ff3b3b", color: "#fff", border: "none", padding: "12px", borderRadius: "30px", fontWeight: "bold", fontSize: "1rem", cursor: "pointer", }}> Acompanhar entrega </button>
                            <button onClick={confirmarEntregaFinal} style={{ flex: 1, backgroundColor: "#27ae60", color: "#fff", border: "none", padding: "12px", borderRadius: "30px", fontWeight: "bold", fontSize: "1rem", cursor: "pointer", }}> Concluir Pedido </button>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: "40px" }}>
                    ...
                    {pedidosAnteriores.length > 0 ? (
                      pedidosAnteriores.map((pedidoAntigo, index) => (
                        <div key={index} style={{ backgroundColor: "#fbeceb", padding: "25px", borderRadius: "15px", marginBottom: "15px", }}>
                          <div style={{ display: "flex", gap: "25px" }}>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: "120px", }}>
                              <div style={{ width: "120px", height: "120px", backgroundColor: "#bdc3c7", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", }}>
                                <img src={pedidoAntigo.itens[0]?.imagem || imgPlaceholder} alt="Pedido" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "10px", }} />
                              </div>
                              <span style={{ marginTop: "10px", fontWeight: "bold", color: "#333", }}> R$ {calcularTotalComTaxa(pedidoAntigo).toFixed(2).replace(".", ",")} </span>
                            </div>

                            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px", }}>
                              <span style={{ fontWeight: "bold", color: "#a82424", fontSize: "1.1rem", }}> {pedidoAntigo.restaurante_nome} </span>
                      ...
                      <div style={{ padding: "20px", textAlign: "center", color: "#888", fontWeight: "bold", }}> Nenhum pedido anterior encontrado. </div>
                    )}
                  </div>
                </div>
              </div>
            ...
            {telaAtual === "tela-restaurante" && lojaSelecionada && (
              <div style={{ animation: "fadeIn 0.3s ease-in-out" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "30px", }}>
                  <div style={{ width: "100px", height: "100px", backgroundColor: "#999", borderRadius: "50%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden", }}>
                    <img src={lojaSelecionada.imagem} alt={lojaSelecionada.nome} style={{ width: "100%", height: "100%", objectFit: "cover", }} />
                  </div>
                  <div> <h2 style={{ fontSize: "1.8rem", fontWeight: "bold", color: "#000", margin: 0, }}> {lojaSelecionada.nome} </h2> <p style={{ color: "#777", margin: "5px 0 0 0" }}>{lojaSelecionada.especialidade}</p> </div>
                </div>
                ...
                <div style={{ marginBottom: "50px" }}>
                  ...
                  <div style={{ backgroundColor: "#ffe6e8", padding: "30px", borderRadius: "10px", display: "flex", alignItems: "center", position: "relative", }}>
                      ...
                      {produtos
                        ...
                        .map(item => (
                          <div key={item.id} style={{ backgroundColor: "#fff", padding: "15px", borderRadius: "8px", minWidth: "150px", }}>
                            ...
                            <p style={{ fontSize: "0.7rem", color: "#999", textDecoration: "line-through", margin: "0 0 2px 0", }}> R$ {(item.preco * 1.2).toFixed(2)} </p>{" "}
                            {/* Preço fake maior */}
                    ...
                    {/* Botão circular de seta */}
                    <div style={{ width: "40px", height: "40px", border: "2px solid #fff", borderRadius: "50%", display: "flex", justifyContent: "center", alignItems: "center", position: "absolute", right: "15px", cursor: "pointer", color: "#fff", }}> &gt; </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "30px", marginBottom: "40px", }}>
                    {produtos
                      .filter(p => Number(p.id_restaurante) === Number(lojaSelecionada?.id))
                      .map(produto => (
                        <div key={produto.id || produto.nome} onClick={() => { setProdutoSelecionado(produto); setQuantidadeProduto(1); }} style={{ backgroundColor: "#ffe6e8", padding: "20px", borderRadius: "10px", display: "flex", gap: "20px", alignItems: "center", cursor: "pointer", }}>
                          ...
                          {/* Botão de + no cartão */}
                          <button style={{ backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "50%", width: "40px", height: "40px", fontSize: "1.5rem", display: "flex", justifyContent: "center", alignItems: "center", cursor: "pointer", }}> + </button>
                        </div>
                      ))}
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "center", marginTop: "20px", }}>
                    <button onClick={() => setTelaAtual("cadastro-produto")} style={{ width: "60px", height: "60px", borderRadius: "50%", backgroundColor: "#999", color: "#fff", fontSize: "2.5rem", border: "none", cursor: "pointer", display: "flex", justifyContent: "center", alignItems: "center", transition: "transform 0.2s", boxShadow: "0 4px 10px rgba(0,0,0,0.1)", }} onMouseEnter={e => (e.currentTarget.style.transform = "scale(1.1)")} onMouseLeave={e => (e.currentTarget.style.transform = "scale(1)")} title="Adicionar novo produto"> + </button>
                  </div>
                </div>
              </div>
            )}
      ...
      {produtoSelecionado && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0, 0, 0, 0.4)", zIndex: 99999, display: "flex", justifyContent: "center", alignItems: "center", }}>
          <div style={{ backgroundColor: "#fff", borderRadius: "15px", padding: "40px", width: "750px", maxWidth: "90%", display: "flex", gap: "30px", position: "relative", boxShadow: "0 10px 25px rgba(0,0,0,0.2)", }}>
            {/* Botão Fechar (X vermelho) */}
            <span onClick={() => setProdutoSelecionado(null)} style={{ position: "absolute", top: "15px", right: "20px", color: "#ff3b3b", fontSize: "1.5rem", fontWeight: "bold", cursor: "pointer", }}> X </span>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              <h2 style={{ fontSize: "1.4rem", margin: "0 0 10px 0", textAlign: "center", color: "#000", fontWeight: "bold", }}> {produtoSelecionado.nome} </h2>
              <p style={{ color: "#555", fontSize: "1.1rem", margin: "0 0 10px 0", fontWeight: "500", }}> {produtoSelecionado.descricao} </p>
              <h3 style={{ fontSize: "1.4rem", color: "#ff3b3b", margin: "0 0 20px 0", }}> R${produtoSelecionado.preco.toFixed(2).replace(".", ",")} </h3>
              ...
              <div style={{ border: "2px solid #aaa", borderRadius: "8px", padding: "10px 15px", marginBottom: "15px", }}>
                ...
                <div style={{ color: "#555", fontSize: "0.9rem", fontWeight: "500", display: "flex", alignItems: "center", gap: "5px", }}> <span>Tempo de espera: </span> <span>{dadosEntrega?.tempoEstimado || "30-40 min"}</span> </div>
              </div>

              {/* Campo de Comentários */}
              <div style={{ marginBottom: "10px" }}>
                <label style={{ display: "block", marginBottom: "8px", fontWeight: "bold", color: "#555", fontSize: "1rem", }}> Algum Comentario? </label>
                <textarea value={comentario} onChange={e => setComentario(e.target.value)} placeholder="EX: Tirar o coco ralado, paçoca, amendoim" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "2px solid #aaa", resize: "none", height: "60px", fontFamily: "inherit", boxSizing: "border-box", fontSize: "0.95rem", fontWeight: "bold", color: "#555", }} />
              </div>

              <p style={{ color: "#ff3b3b", fontSize: "0.9rem", textAlign: "right", margin: "0 0 15px 0", cursor: "pointer", fontWeight: "bold", }}> Denunciar item </p>

              {/* Controles de Quantidade e Botão Adicionar */}
              <div style={{ display: "flex", gap: "15px", marginTop: "auto" }}>
                {/* Botões + e - */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", border: "2px solid #ff3b3b", borderRadius: "8px", padding: "10px 15px", width: "130px", }}>
                  {/* Sinal de MAIS (Aumenta a quantidade) */}
                  <span onClick={() => setQuantidadeProduto(q => q + 1)} style={{ color: "#ff3b3b", fontSize: "1.6rem", fontWeight: "bold", cursor: "pointer", userSelect: "none", }}> + </span>

                  <span style={{ fontSize: "1.3rem", fontWeight: "bold", color: "#ff3b3b", userSelect: "none", }}> {quantidadeProduto} </span>

                  {/* Sinal de MENOS (Diminui a quantidade, mas não deixa passar de 1) */}
                  <span onClick={() => setQuantidadeProduto(q => Math.max(1, q - 1))} style={{ color: "#ff3b3b", fontSize: "1.6rem", fontWeight: "bold", cursor: "pointer", userSelect: "none", }}> - </span>
                </div>

                {/* Botão Adicionar ao Carrinho */}
                <button onClick={handleAdicionarAoCarrinho} style={{ flex: 1, backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "8px", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", fontSize: "1.1rem", fontWeight: "bold", cursor: "pointer", }}> <span>Adicionar</span> <span>R$ {(produtoSelecionado.preco * quantidadeProduto).toFixed(2).replace(".", ",")}</span> </button>
              </div>
            </div>
          </div>
        </div>
      ...
      {modalEnderecoAberto && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0, 0, 0, 0.4)", zIndex: 99999, display: "flex", justifyContent: "center", alignItems: "center", }}>
          <div style={{ backgroundColor: "#fff", borderRadius: "15px", padding: "40px", width: "500px", maxWidth: "90%", display: "flex", flexDirection: "column", position: "relative", boxShadow: "0 10px 25px rgba(0,0,0,0.2)", }}>
            {/* Botão Fechar */}
            <span onClick={() => setModalEnderecoAberto(false)} style={{ position: "absolute", top: "15px", right: "20px", color: "#ff3b3b", fontSize: "1.5rem", fontWeight: "bold", cursor: "pointer", }}> X </span>
            ...
            {passoEndereco === 1 && (
              <div style={{ animation: "fadeIn 0.3s ease-in-out" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "15px", }}>
                  {sugestoes.length > 0 && (
                    <div style={{ backgroundColor: "#fff", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", marginTop: "-25px", marginBottom: "20px", maxHeight: "200px", overflowY: "auto", }}>
                      {sugestoes.map((item, index) => (
                        <div
                          ...
                          style={{ padding: "12px", borderBottom: "1px solid #eee", cursor: "pointer", fontSize: "0.9rem", color: "#333", }}>
                          📍 {item.display_name}
                        </div>
                      ))}
                    </div>
            ...
            {passoEndereco === 2 && (
              <div style={{ animation: "fadeIn 0.3s ease-in-out" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "30px", }}>
                  <input type="text" placeholder="informe o numero" value={semNumero ? "S/N" : numero} disabled={semNumero} onChange={e => setNumero(e.target.value)} style={{ border: "1px solid #ccc", borderRadius: "8px", padding: "15px", width: "250px", textAlign: "center", fontSize: "1.1rem", marginBottom: "15px", }} />
            ...
            {passoEndereco === 3 && enderecoSelecionado?.lat && (
              <div style={{ animation: "fadeIn 0.3s ease-in-out" }}>
                ...
                {enderecoSelecionado?.lat && enderecoSelecionado?.lon ? (
                  <div style={{ width: "100%", height: "250px", backgroundColor: "#aaa", borderRadius: "15px", marginBottom: "30px", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden", }}>
                    <MapContainer center={[enderecoSelecionado.lat, enderecoSelecionado.lon]} zoom={18} style={{ height: "300px", width: "100%" }}>
                      <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                      <Marker
                        position={[enderecoSelecionado.lat, enderecoSelecionado.lon]} draggable={true}
                        eventHandlers={{
                          dragend: e => {
                  ...
                  <div style={{ height: "300px", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#f0f0f0", borderRadius: "15px", marginBottom: "20px", }}> <p>Carregando mapa...</p> </div>
                )}
                <button onClick={() => setPassoEndereco(4)} style={{ width: "100%", backgroundColor: "#ff4d6d", color: "#fff", border: "none", borderRadius: "10px", padding: "15px", fontWeight: "bold", fontSize: "1.1rem", cursor: "pointer", boxShadow: "0 4px 10px rgba(255, 77, 109, 0.2)", }}> Confirmar localização </button>
              </div>
            )}
            ...
            {passoEndereco === 4 && (
              <div style={{ animation: "fadeIn 0.3s ease-in-out" }}>
                ...
                <p style={{ textAlign: "center", fontWeight: "bold", fontSize: "0.95rem", marginBottom: "30px", color: "#000", }}> Endereço cadastrado | Bairro | Estado </p>
                ...
                <div style={{ display: "flex", gap: "15px", marginBottom: "40px" }}>
                  <button onClick={() => setTipoFavorito("casa")} style={{ flex: 1, padding: "12px", borderRadius: "8px", border: tipoFavorito === "casa" ? "2px solid #ff4d6d" : "1px solid #ccc", backgroundColor: tipoFavorito === "casa" ? "#fff0f3" : "#fff", color: tipoFavorito === "casa" ? "#ff4d6d" : "#777", fontWeight: "bold", display: "flex", justifyContent: "center", alignItems: "center", gap: "10px", cursor: "pointer", transition: "all 0.2s", }}> 🏠 Casa </button>

                  <button onClick={() => setTipoFavorito("trabalho")} style={{ flex: 1, padding: "12px", borderRadius: "8px", border: tipoFavorito === "trabalho" ? "2px solid #ff4d6d" : "1px solid #ccc", backgroundColor: tipoFavorito === "trabalho" ? "#fff0f3" : "#fff", color: tipoFavorito === "trabalho" ? "#ff4d6d" : "#777", fontWeight: "bold", display: "flex", justifyContent: "center", alignItems: "center", gap: "10px", cursor: "pointer", transition: "all 0.2s", }}> 📄 Trabalho </button>
                </div>

                <button onClick={salvarEnderecoCompleto} style={{ width: "100%", backgroundColor: "#ff4d6d", color: "#fff", border: "none", borderRadius: "10px", padding: "15px", fontWeight: "bold", fontSize: "1.1rem", cursor: "pointer", boxShadow: "0 4px 10px rgba(255, 77, 109, 0.2)", }}> Confirmar localização </button>
              </div>
            )}
          </div>
        </div>
      ...
      {modalEnderecos && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999, }}>
          <div style={{ backgroundColor: "#fff", borderRadius: "15px", padding: "30px", width: "90%", maxWidth: "500px", maxHeight: "80vh", overflowY: "auto", boxShadow: "0 10px 40px rgba(0,0,0,0.2)", }}>
            {/* Cabeçalho do Modal */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "25px", }}>
              <h3 style={{ margin: 0, color: "#ff3b3b", fontSize: "1.3rem" }}>📍 Selecione o endereço de entrega</h3>
              <span onClick={() => setModalEnderecos(false)} style={{ fontSize: "1.5rem", cursor: "pointer", color: "#999", fontWeight: "bold", }}> ✕ </span>
            </div>
            ...
            {listaEnderecos.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", }}>
                {listaEnderecos.map(endereco => (
                  <div
                    key={endereco.id} onClick={() => selecionarEndereco(endereco)} style={{ padding: "15px", border: `2px solid ${endereco.id === enderecoSelecionado?.id ? "#ff3b3b" : "#eaeaea"}`, borderRadius: "10px", cursor: "pointer", backgroundColor: endereco.id === enderecoSelecionado?.id ? "#fff5f5" : "#fff", transition: "all 0.2s", }}
                    onMouseEnter={e => {
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "5px", }}>
                      ...
                      <div>
                        <p style={{ fontWeight: "bold", margin: 0, fontSize: "1rem", }}> {endereco.logradouro}, {endereco.numero || "S/N"} </p>
                        <p style={{ margin: "3px 0 0 0", color: "#666", fontSize: "0.9rem", }}> {endereco.bairro} - {endereco.cidade}/{endereco.estado} </p>
                      </div>
                    </div>

                    {endereco.complemento && ( <p style={{ margin: "5px 0 0 35px", fontSize: "0.8rem", color: "#999", fontStyle: "italic", }}> "{endereco.complemento}" </p> )}

                    {endereco.id === enderecoSelecionado?.id && ( <p style={{ margin: "10px 0 0 35px", fontSize: "0.8rem", color: "#ff3b3b", fontWeight: "bold", }}> ✅ Endereço atual </p> )}
                  </div>
                ))}
              </div>
            )}

            <button onClick={() => setModalEnderecos(false)} style={{ width: "100%", padding: "12px", marginTop: "20px", backgroundColor: "#f5f5f5", color: "#333", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", fontSize: "1rem", }}> Cancelar </button>
          </div>
        </div>
      )}
          ...
          <div style={{ backgroundColor: "#fff", padding: "40px", borderRadius: "20px", textAlign: "center", width: "400px", boxShadow: "0 10px 30px rgba(0,0,0,0.2)", }}>
                  ...
                  onClick={() => {
                    if (itensCarrinho.length > 0) {
                      setPedidoAtual({
                        ...
                        total: (itensCarrinho.reduce((acc, i) => acc + Number(i.preco_unitario) * Number(i.quantidade), 0) + (dadosEntrega.isFreteGratis ? 0 : Number(dadosEntrega.taxaEntrega || 0))).toFixed(2).replace(".", ","),
                      });
                    }
                    setModalPagamentoAberto(false);
                    setTelaAtual("pedidos");
            ...
            {statusPagamento === "erro" && (
              <div>
                ...
                <button style={{ marginTop: "30px", padding: "12px 30px", backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "25px", fontWeight: "bold", cursor: "pointer", width: "100%", }} onClick={() => setModalPagamentoAberto(false)}> Tentar novamente </button>
              </div>
            )}
          </div>
        </div>
      ...
      <ModalPix
        ...
        onPagamentoConfirmado={() => { setItensCarrinho([]); setPedidoAtivoId(null); localStorage.removeItem("pop_pedido_id"); carregarHistoricoPedidos(); toast.success("✅ Pagamento confirmado! Seu pedido está sendo preparado."); setTelaAtual("pedidos"); }}
      />

      <ModalAcompanharEntrega isOpen={modalAcompanharAberto} onClose={() => setModalAcompanharAberto(false)} pedidoAtivoId={pedidoAtual?.id} infoEntregador={infoEntregador} />

      <ModalConfirmacaoRetirada
        isOpen={modalConfirmacaoRetiradaAberto}
        onClose={() => { setModalConfirmacaoRetiradaAberto(false); setItensCarrinho([]); setPedidoAtivoId(null); }}
        pedidoConfirmado={pedidoConfirmadoRetirada}
        onVerPainel={() => { setModalConfirmacaoRetiradaAberto(false); setModalPainelSenhasAberto(true); }}
      />

      <ModalPainelSenhas
        isOpen={modalPainelSenhasAberto}
        onClose={() => { setModalPainelSenhasAberto(false); setItensCarrinho([]); setPedidoAtivoId(null); localStorage.removeItem("pop_pedido_id"); setTelaAtual("pedidos"); }}
      ...
      {modalRelatorioAberto && (
        <div
          style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0, 0, 0, 0.5)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center", }}>
          <div style={{ width: "95%", maxWidth: "1200px", height: "90vh", backgroundColor: "#fdf2f5", borderRadius: "15px", overflow: "hidden", position: "relative" }}>
            <button onClick={() => setModalRelatorioAberto(false)} style={{ position: "absolute", top: "15px", right: "20px", zIndex: 10, backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "50%", width: "35px", height: "35px", fontSize: "1.2rem", cursor: "pointer", fontWeight: "bold", }}> ✕ </button>
      ...
      {modalPerfilAberto && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0, 0, 0, 0.5)", zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center", }}>
          <div
            style={{ width: "95%", maxWidth: "700px", maxHeight: "90vh", backgroundColor: "#fdf2f5", borderRadius: "24px", overflow: "hidden", position: "relative", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1)", }}>
            <button onClick={() => setModalPerfilAberto(false)} style={{ position: "absolute", top: "20px", right: "20px", zIndex: 10, backgroundColor: "#ff3b3b", color: "#fff", border: "none", borderRadius: "50%", width: "35px", height: "35px", fontSize: "1.2rem", cursor: "pointer", fontWeight: "bold", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(255, 59, 59, 0.3)", transition: "all 0.2s", }} onMouseEnter={e => { e.currentTarget.style.transform = "scale(1.1)"; e.currentTarget.style.backgroundColor = "#ff1f1f"; }} onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.backgroundColor = "#ff3b3b"; }}> ✕ </button>
            <div style={{ height: "100%", maxHeight: "90vh", overflowY: "auto", padding: "10px" }}>
              <PerfilUsuario onClose={() => setModalPerfilAberto(false)} setTelaAtual={setTelaAtual} />
            </div>
          </div>
        </div>
      )}
