...
class ItemPedido(db.Model):
    ...
    # Converte o item para dicionário para serialização JSON
    def to_dict(self):
        return {
            "id": self.id,
...
class Pedido(db.Model):
    ...
    # Inicia a simulação de preparo e entrega gerando tempos aleatórios
    ...
    # Gera um código de confirmação usando os últimos 4 dígitos do telefone do usuário
    ...
    # Gera uma senha numérica de 3 dígitos para retirada no balcão
    def gerar_senha_retirada(self):
        self.numero_senha = str(random.randint(100,999))
        return self.numero_senha

    # Atualiza o total do pedido somando o subtotal de todos os itens
    def atualizar_total(self):
        self.total = sum(item.subtotal() for item in self.itens)

    # Converte o pedido completo para dicionário para serialização JSON
    def to_dict(self):
        return {
            "id": self.id,
...
# Cria um novo carrinho de compras (pedido em aberto) para um usuário em um restaurante
...
# Adiciona um item ao carrinho usando os dados reais do produto no banco de dados
def adicionar_item_ao_carrinho(pedido_id, dados_item):
    try:
        ...
        novo_item = ItemPedido(
            nome_doce=doce_real.nome,
            quantidade=dados_item['quantidade'],
            preco_unitario=doce_real.preco
        )
...
# Remove um item específico do carrinho e retorna o pedido atualizado
def remover_item_do_carrinho(item_id):
    from .pedido_model import ItemPedido, Pedido
